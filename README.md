...
- **Server + CLI**: `finidb serve` (node:http, multi-database, users/grants with scrypt, Basic
  and bearer auth, the REST routes of doc 07, long-poll `/changes`, CSV load with append / upsert / replace and a dry run, linked tables fetched from an API (`source`, the `fmp` preset, `/refresh`), batch), `finidb
