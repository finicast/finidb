...
/** The workbook writer, registered by the Node entry point (it needs zlib); see `exportXlsx`. */
export type XlsxOptions = Record<string, unknown>;
export type XlsxResult = { buffer: Uint8Array; sheets: string[]; formulas: number; values: number; notes: string[] } & Record<string, unknown>;
let xlsxWriter: ((db: Database, modelId: string, opts: XlsxOptions) => XlsxResult) | undefined;
export function registerXlsxWriter(w: (db: Database, modelId: string, opts: XlsxOptions) => XlsxResult) { xlsxWriter = w; }
...
export class FiniDB {
   ...
   *  The writer is registered by the Node entry point; `finidb/browser` leaves it out. */
  exportXlsx(modelId: string, opts: XlsxOptions = {}): XlsxResult {
    if (!xlsxWriter) throw new Error('XLSX_UNAVAILABLE: import finidb rather than finidb/browser to write a workbook');
    return xlsxWriter(this.db, modelId, opts);
  }
