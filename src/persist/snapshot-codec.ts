/**
 * The snapshot format itself, with nothing but the language underneath it: no filesystem, no Buffer, so the
 * same encoder and decoder run on a server, in a worker and in a browser tab.
 *
...
import { FiniDB, ReferenceEvaluator } from '../core.js';
...
class Payload {
  ...
  json(v: unknown): number { return this.add('json', utf8.encode(JSON.stringify(v))); }
}
...
/** Serialize `f` (inputs only) into the snapshot container's bytes. */
export function encodeSnapshot(f: FiniDB, opts: { seq?: number } = {}): Uint8Array {
  ...
  let size = 8 + headerBytes.length;
  for (const c of payload.chunks) size += c.length;
  ...
  for (const c of payload.chunks) { out.set(c, at); at += c.length; }
  return out;
}
...
export function readSnapshotBytes(bytes: Uint8Array): { header: SnapshotHeader; payload: Uint8Array } {
  if (bytes.length < 8 || String.fromCharCode(bytes[0], bytes[1], bytes[2], bytes[3]) !== MAGIC) throw new Error('SNAPSHOT_BAD_MAGIC');
  const hlen = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength).getUint32(4, true);
  const header = JSON.parse(utf8d.decode(bytes.subarray(8, 8 + hlen))) as SnapshotHeader;
  if (header.version !== FORMAT_VERSION) throw new Error(`SNAPSHOT_BAD_VERSION: ${header.version}`);
  return { header, payload: bytes.subarray(8 + hlen) };
}

/** Copy a section into a fresh typed array (works whatever the buffer alignment). */
function section<T extends Float64Array | Int32Array | Uint8Array>(payload: Uint8Array, s: SectionRef, ctor: new (n: number) => T, bytesPer: number): T {
  const out = new ctor(s.length / bytesPer);
  new Uint8Array(out.buffer).set(payload.subarray(s.offset, s.offset + s.length));
  return out;
}
function sectionJson<T>(payload: Uint8Array, s: SectionRef): T { return JSON.parse(utf8d.decode(payload.subarray(s.offset, s.offset + s.length))) as T; }

function fillColumn(f: Field, h: FieldHeader, n: number, secs: SectionRef[], payload: Uint8Array) {
  const c = f.column;
...
/** Rebuild a working FiniDB from the bytes of a snapshot. */
export function decodeSnapshot(bytes: Uint8Array, opts: { engine?: 'incremental' | 'reference' } = {}): FiniDB {
  const { header, payload } = readSnapshotBytes(bytes);
